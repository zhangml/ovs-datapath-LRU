...
void ovs_dp_process_packet(struct sk_buff *skb, struct sw_flow_key *key)
{
	...
	//pr_info("version-byzml\n");
    //printk("version-byzml\n");
    
    sprintf(id, "%08x%08x%02x%04x%04x", key->ipv4.addr.src, key->ipv4.addr.dst, key->ip.proto, key->tp.src, key->tp.dst);
    //printk("%s\n", id);
...
static int __init dp_init(void)
{
    ...
    err = init_LRU(8000);
    //pr_info("init_bloom_sketch success\n");
